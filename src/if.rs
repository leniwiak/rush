...
const SUPER_COMMANDS:[&str;3] = ["and", "or", "not"];
...
fn main() {
    ...
    let commands = rush::split_commands(opts.clone(), Vec::from(["and", "or", "not"]));
    // Collect exit statuses here
    let mut returns: HashMap<usize, rush::CommandStatus> = HashMap::new();

    // Run all standard commands
    let mut index = 0;
    while index < commands.len() {
        // Execute all standard commands and collect their statuses
        if !SUPER_COMMANDS.contains(&commands[index][0].as_str()) {
...
fn silent_exec(args:&[String], index:usize, returns:&mut HashMap<usize, rush::CommandStatus>) {
    ...
    match process::Command::new(&args[0]).args(&args[1..]).stdout(Stdio::null()).status() { 
        Err(e) => {
            ...
            rush::report_failure(index, returns)
        },
        Ok(process) => {
            // If the command is possible to run, save it's status to "returns" variable
            let command_status = rush::CommandStatus {
                code: process.code(),
...
fn and(index_of_and:usize, returns: &mut HashMap<usize, rush::CommandStatus>) {
    if index_of_and == 0 {
        eprintln!("SYNTAX ERROR! Operator \"AND\" doesn't work when there is nothing before it!");
        rush::report_failure(index_of_and, returns);
        process::exit(1);
    }
    ...
    if prev_status && next_status {
        rush::report_success(index_of_and, returns);
    } else {
        rush::report_failure(index_of_and, returns);
    }
}
...
fn or(index_of_or:usize, returns: &mut HashMap<usize, rush::CommandStatus>) {
    if index_of_or == 0 {
    ...
    if returns.get(&prev).unwrap().success || returns.get(&next).unwrap().success {
        rush::report_success(index_of_or, returns);
        // Overwrite the status of both exit codes to fool the if (or any other) logic that every command is ok
        rush::report_success(prev, returns);
        rush::report_success(next, returns);
    } else {
        rush::report_failure(index_of_or, returns);
    }
}

// This changes the return code after it
fn not(index_of_not:usize, returns: &mut HashMap<usize, rush::CommandStatus>) {
    ...
    rush::report_success(index_of_not, returns);
    ...
    if returns.get(&next).unwrap().success {
        ...
        rush::report_failure(next, returns);
    } else {
        rush::report_success(next, returns);
    }
}